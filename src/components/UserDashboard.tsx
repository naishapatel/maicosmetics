...
export function UserDashboard() {
  ...
  return (
    <Card className="w-full bg-white/60 backdrop-blur-sm border border-mai-mauve/20 shadow-md">
      <CardHeader>
        <CardTitle className={`${isMobile ? 'text-xl' : 'text-2xl'} font-serif text-mai-brown`}>Your Dashboard</CardTitle>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className={`grid w-full grid-cols-2 bg-mai-mauve/10 rounded-full p-1 ${isMobile ? 'text-xs' : ''}`}>
            <TabsTrigger 
              value="recommendations" 
              className="rounded-full data-[state=active]:bg-mai-mauve data-[state=active]:text-white"
            >
              {isMobile ? 'Recommendations' : 'Your Recommendations'}
            </TabsTrigger>
            <TabsTrigger 
              value="saved" 
              className="rounded-full data-[state=active]:bg-mai-mauve data-[state=active]:text-white"
            >
              {isMobile ? 'Saved' : 'Saved Items'}
            </TabsTrigger>
          </TabsList>
            ...
            {recommendations.length > 0 ? (
                ...
                {recommendations.map((item) => (
                  <Card key={item.id} className="overflow-hidden hover:shadow-md transition-shadow bg-white/80 border-mai-mauve/10">
                    <CardContent className="p-4">
                      ...
                      <p className={`text-sm text-gray-500 ${isMobile ? 'line-clamp-1' : ''}`}>{item.brand}</p>
                      <p className="text-mai-mauve mt-1">{item.price}</p>
                      <div className="flex mt-2 gap-1 flex-wrap">
                        {item.ethical_values.slice(0, isMobile ? 2 : 3).map((value, i) => (
              ...
              <div className="text-center py-6 space-y-4 bg-white/40 rounded-lg p-6">
                ...
                <Button 
                  ...
                  className={`bg-mai-mauve hover:bg-mai-mauveDark text-white ${isMobile ? 'px-4 py-2 text-sm' : 'px-6 py-2'} rounded-full`}
                >
                  Take Skin Quiz
                </Button>
              </div>
                    ...
                    <CardContent className="p-4">
                      <div className="flex justify-between">
                        <div className={isMobile ? 'w-5/6' : ''}>
                          <h3 className="font-serif font-semibold text-mai-brown truncate">{item.name}</h3>
                          <p className={`text-sm text-gray-500 ${isMobile ? 'line-clamp-1' : ''}`}>{item.brand}</p>
                          <p className="text-mai-mauve mt-1">{item.price}</p>
                        </div>
                        <Button 
                      ...
                      <div className="flex mt-2 gap-1 flex-wrap">
                        {item.ethical_values.slice(0, isMobile ? 1 : 3).map((value, i) => (
                          <span key={i} className="text-xs bg-mai-mauve/10 text-mai-mauve px-2 py-1 rounded-full">
                            {value}
                          </span>
                        ))}
