  ...
  const fetchPendingPosts = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setPendingPosts([]);
        console.log("No pending posts found or data is null/empty");
      }
    } catch (error) {
        ...
        <BlogPostError error={fetchError} onRetry={handleRetry}>
          <div className="mt-2 p-3 bg-red-50 border border-red-100 rounded text-sm">
            ...
            <ul className="list-disc pl-5 mt-1">
              <li>Are logged in as an admin user</li>
              <li>Have correct permissions to access the blog_post_approvals table</li>
              <li>Check if your admin email is correctly configured in Admin.tsx</li>
            </ul>
          </div>
        </BlogPostError>
      )}
