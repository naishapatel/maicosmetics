...
import { ExternalLink, Leaf, GlobeIcon, Heart, Brain, Smile, HandHelping } from "lucide-react";
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from "@/components/ui/card";
import { Link } from "react-router-dom";

const AwarenessSection = () => {
  const environmentalNonprofits = [
    {
  ...
  const mentalHealthNonprofits = [
    {
      name: "Mental Health America",
      description: "Promoting mental health and preventing mental illness through advocacy, education, and support.",
      url: "https://www.mhanational.org/",
      icon: <Brain className="w-8 h-8 text-mai-darkRed" />,
      category: "Mental Health Advocacy"
    },
    {
      name: "National Alliance on Mental Illness",
      description: "The nation's largest grassroots mental health organization dedicated to building better lives for those affected by mental illness.",
    ...
    },
    {
      ...
      icon: <Smile className="w-8 h-8 text-mai-darkRed" />,
    ...
    },
    {
      ...
      icon: <HandHelping className="w-8 h-8 text-mai-darkRed" />,
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-3xl md:text-4xl font-bold text-mai-brown mb-4">Awareness & Mental Health Organizations</h2>
        <p className="text-lg text-gray-600 max-w-3xl mx-auto">
          We're committed to supporting organizations that promote sustainability, ethical practices, 
          mental well-being, and positive change in the beauty industry and beyond.
        </p>
      </motion.div>

      <div className="mb-16">
        <h3 className="text-2xl font-bold text-mai-brown mb-6">Environmental Organizations</h3>
        <div className="grid md:grid-cols-3 gap-6">
          {environmentalNonprofits.map((nonprofit, index) => (
            <motion.div
              key={nonprofit.name}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
      ...
      <div className="mb-12">
        <h3 className="text-2xl font-bold text-mai-brown mb-6">Mental Health Organizations</h3>
        <div className="grid md:grid-cols-2 gap-6">
          {mentalHealthNonprofits.map((nonprofit, index) => (
            <motion.div
              key={nonprofit.name}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              ...
              transition={{ duration: 0.5, delay: index * 0.1 }}
            >
              <Card className="h-full bg-mai-cream/20 border-mai-mauve/10 hover:border-mai-mauve/30 transition-all duration-300">
                <CardHeader className="pb-2">
                  <div className="mb-3">{nonprofit.icon}</div>
                  <CardTitle className="text-mai-brown">{nonprofit.name}</CardTitle>
                  <span className="text-xs font-medium bg-mai-cream text-mai-darkRed px-2 py-1 rounded-full inline-block mt-1">
                    {nonprofit.category}
                  </span>
                </CardHeader>
                <CardContent className="pb-4">
      ...
      >
        ...
        <p className="text-gray-700 mb-4">
          The beauty industry is evolving towards more sustainable and ethical practices with a growing focus on mental well-being. 
